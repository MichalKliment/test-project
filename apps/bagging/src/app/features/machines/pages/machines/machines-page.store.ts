...
export interface MachinesPageState {
  data: MachinesType | null;
  loading: boolean;
}

@Injectable()
export class MachinesPageStore extends ComponentStore<MachinesPageState> {
  ...
  private readonly updateData = this.updater((state, input: MachinesType) => ({
    ...state,
    data: input,
  }));
